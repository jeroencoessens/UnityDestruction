...
public class ToolUser : MonoBehaviour {
    ...
    {
        var hits = Physics.RaycastAll(transform.position, transform.forward, 300.0f);
        hits.Reverse();
        ...
        foreach (var hit in hits)
        {
            ...
            if (UseShuriken)
            {
                if (hit.distance > 180) continue;
                yield return new WaitForSeconds(hit.distance / 400.0f);
            }
            else if (!UseShuriken)
            {
                if (hit.distance > 6) continue;
            }
